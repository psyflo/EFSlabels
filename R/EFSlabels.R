...
dataFileLocation <- "data/data_project_206763_2015_06_11.csv"
codebookFileLocation <- "data/codebook_project_206763_2015_09_23.csv"
...
EFSlabels <- function(dataFileLocation, codebookFileLocation) {
  ...
  for (name in colnames(df)) { # do this for each comlumname
    loc <- grep(paste("\t",name,"\t",sep=""), df_names) # find location of column name in file

    if (length(loc)==1) {       # do this only if a column name was found
      i <- 1 # start value, go through all possible levels

      lvlvect <- c()
      while (df_names[loc+i] != "" & length(grep("v_",df_names[loc+i], invert = TRUE)) != 0) {
        # enter this loop only if a subsequent row is not empty (this indicates change of page)
        # and if next row does not include a "v_" (this indicates a new question)
